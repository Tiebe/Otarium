...
internal fun AveragesScreen(component: AveragesComponent) {
    Column(Modifier.fillMaxSize()) {
        val subjects = component.gradesList.subscribeAsState().value.map { it.grade.subject }.distinct().sortedBy { it.description.lowercase() }
                ...
                Surface(modifier = Modifier.fillMaxSize().padding(it)) {
                    when (val instance = child.instance) {
                        ...
                        is AveragesComponent.Child.SubjectChild -> AverageSubjectPopup(
                            ...
                            component.gradesList.subscribeAsState().value.filter { it.grade.subject.id == instance.subjectId}
                        )
                    }
                }
            }
